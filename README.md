...
directory or resource is considered a Node in the Book(the Book itself is also a Node). To render a specific type of content a renerer or parser for that type must be registred. A **`ContentRenderer'**. Volta provides the following Renderers
